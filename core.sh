...
# Módulo de verifcação usado especialmente pelo spm em suas
...
_MANAGE_SCRIPTS_AND_ARCHIVES()
{
    ...
    pushd "/info/" &>/dev/null
    if mv 'desc' "/var/lib/spm/desc/${packname}.desc"; then
    ...
    if [[ -e "rm.sh" ]]; then # rm.sh existe?
        if mv 'rm.sh' "/var/lib/spm/remove/${packname}.rm"; then
            echo -e "${blue}[MOVED REMOVE]${end}\tto /var/lib/spm/remove/${packname}.rm"
        else
            echo -e "${red}[ERROR!]${end} could not move rm.sh to /var/lib/spm/remove/${packname}.rm"
            echo "ABORTING..."
            exit 1
        fi
    fi
  ...
  ( # Subshell marota
    ...
    # Se alguma variavel estiver nula não podemos continuar.
        ...
        if [[ "$pkgname" != "$(echo "$package" | cut -d '-' -f '1')" ]]; then
            ...
            exit 1
        elif [[ "$version" != $(echo "$package" | cut -d '-' -f '2' | sed 's/\.slp//') ]]; then
            ...
            exit 1
        elif [[ "$build" != $(echo "$package" | cut -d '-' -f '3' | sed 's/\.slp//') ]]; then
...
_LIST_ARCHIVES_DIRECTORIES()
{
    ...
    sed -i "
        ...
        /^info\/desc/d
        /info\/rm.sh/d
        /^var\/lib\/spm\/list\/.*\.list/d
        /^var\/lib$/d
        /^var\/lib\/spm/d
        /^var\/lib\/spm\/list/d
        /^var\/lib\/spm\/remove/d
    " "${dirlist}/${packname}"
...
_INSTALL_PKG()
{
  ( # Subshell Suicide
    ...
    # plantpkg -i /tmp/packages/pacote-versao.slp ou
    # plantpkg -i /tmp/*.slp
...
_UPGRADE()
{
  ( # Subshell Meu precioso
    ...
    local dir_desc='/var/lib/spm/desc'
    ...
    pushd "$dir_desc" &>/dev/null  # entrando no diretório /var/lib/spm/desc
    jj='0' # Var Incremento
    ...
    local packname="${packname/%.slp/}" # Cortando extensão para imprimir certinho.
             ...
             if [[ "${name_version_build}.desc" =~ ^${re}.desc ]] || \
                [[ "${name_version_build}.desc" =~ ^${re}.* && "$count_occurrences" -eq '1' ]] ; then
                 ...
                 if [[ -z "$auto_confirm" ]] || [[ "$auto_confirm" != 'y' ]]; then # se ter -y é auto confirmar ;)
                     ...
                     [[ "$confirm" != 'y' ]] && return 0
                     pushd "${dirlist}" 1>/dev/null # Entrando no diretório das listas /var/lib/spm/list
...
_REMOVE_NOW()
{
    ...
    if [[ -e "${dirremove}/${packname}.rm" ]]; then
        ...
        # Removendo script pacote.rm em /var/lib/spm/remove/
        if [[ -e "${dirremove}/${packname}.rm" ]]; then
...
_UPDATE_SPM()
{
    local link='https://github.com/wxkz/spm'
    local connectivity="${red}[FAILED]${end}"
    local tmp_dir_spm="/tmp/${PRG}pkg"
    local DOWNLOAD m
    ...
    files=$(shopt -s nullglob dotglob; echo /tmp/spm/*)
    if (( ${#files} ))
    then
	rm -rf /tmp/spm/;
    fi
    ...
    for m in "${PRG}.conf" "${PRG}.8" 'core.sh' 'help.sh' "$PRG"; do
        ...
        case $m in
            ...
            (spm.conf)
                ...
                answer="${answer:=n}"
                if [[ "$answer" = 'y' ]]; then
                    cp -v "$m" "/etc/spm/" || return 1
                else
                    continue
                fi
             ;;
             (core.sh|help.sh) cp -v "$m" "/usr/libexec/spm/" || return 1 ;;
        esac
    done
    
    [[ -d "${tmp_dir_spm}" ]] && rm -r "${tmp_dir_spm}"
}
